...
void gradSphere( pt *pts, int numpts, nbhd *ptsnbhd, double *gradarray, double s, double radius,int dim, double *maxGradient){
    int i,j ;
    double graddotpt;
    double norm;
    *maxGradient = 0;
    for(i=0;i<numpts;i++){
        ...
        for(j=0;j<dim;j++){
            ...
            norm+=gradarray[i*dim+j]*gradarray[i*dim+j];
        }
        norm = sqrt(norm);
        if (norm > *maxGradient) {
            *maxGradient = norm;
...
void gradBall( pt *pts, int numpts, nbhd *ptsnbhd, double *gradarray, double s, double radius,int dim, double *maxGradient){
    int i,j ;
    double graddotpt;
    double err;
    double normsq;
...
void gradShell( pt *pts, int numpts, nbhd *ptsnbhd, double *gradarray, double s, double radius,int dim,double inner_rad,double *maxGradient){
    int i,j ;
    double graddotpt;
    double err;
    double normsq;
...
double sEnergy(pt *points, double s, double radius, double* cornera, double* cornerb, int numpts, cube *cubes, int numCubes, nbhd *nbhds, int *neighborArray, cube *neighborhood, int maxNbrs, int *trinaryList, nbhd* cubeNbhd, int* testIndex, int* cubePtsArray,int dim, double *minDistance, double *maxGradient){
    ...
    return energy(points, numpts, nbhds, s, radius, dim, minDistance);
}

//finds the energy and gradient of the ball by initializing the structure, then finding the gradient and energy
double sEnGradBall(pt *points, double *theGrad, double s, double radius, double* cornera, double* cornerb, int numpts, cube *cubes, int numCubes, nbhd *nbhds, int *neighborArray, cube *neighborhood, int maxNbrs, int *trinaryList, nbhd* cubeNbhd, int* testIndex, int* cubePtsArray,int dim, double inner_rad, double *minDistance, double *maxGradient){
    
    AssignCubes(radius, cornera, cornerb, points, numpts, cubes, cubePtsArray, maxNbrs,dim);
    AssignNbhds(cubes, radius, numpts, numCubes, points, nbhds, neighborArray, neighborhood, maxNbrs, trinaryList, cubeNbhd, testIndex,dim);
    gradBall(points, numpts, nbhds, theGrad, s, radius,dim, maxGradient);
    return energy(points, numpts, nbhds, s, radius,dim, minDistance);
}

//finds the energy and gradient of the sphere by initializing the structure, then finding the gradient and energy
double sEnGradSphere(pt *points, double *theGrad, double s, double radius, double cornera[3], double cornerb[3], int numpts, cube *cubes, int numCubes, nbhd *nbhds, int *neighborArray, cube *neighborhood, int maxNbrs, int *trinaryList, nbhd* cubeNbhd, int* testIndex, int* cubePtsArray,int dim, double inner_rad, double *minDistance, double *maxGradient){
    AssignCubes(radius, cornera, cornerb, points, numpts, cubes, cubePtsArray, maxNbrs,dim);
    AssignNbhds(cubes, radius, numpts, numCubes, points, nbhds, neighborArray, neighborhood, maxNbrs, trinaryList, cubeNbhd, testIndex,dim);
    gradSphere(points, numpts, nbhds, theGrad, s, radius,dim, maxGradient);
    return energy(points, numpts, nbhds, s, radius,dim, minDistance);
}

//finds the energy and gradient of the shell by initializing the structure, then finding the gradient and energy
double sEnGradShell(pt *points, double *theGrad, double s, double radius, double cornera[3], double cornerb[3], int numpts, cube *cubes, int numCubes, nbhd *nbhds, int *neighborArray, cube *neighborhood, int maxNbrs, int *trinaryList, nbhd* cubeNbhd, int* testIndex, int* cubePtsArray,int dim,double inner_rad, double *minDistance, double *maxGradient){
    AssignCubes(radius, cornera, cornerb, points, numpts, cubes, cubePtsArray, maxNbrs,dim);
    AssignNbhds(cubes, radius, numpts, numCubes, points, nbhds, neighborArray, neighborhood, maxNbrs, trinaryList, cubeNbhd, testIndex,dim);
    gradShell(points, numpts, nbhds, theGrad, s, radius,dim,inner_rad, maxGradient);
    return energy(points, numpts, nbhds, s, radius,dim, minDistance);
}
