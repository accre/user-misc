...
int main(int argc, char* argv[]){
    ...
    double (*sEnGrad)(pt *points, double *theGrad, double s, double radius, double cornera[3], double cornerb[3], int numpts, cube *cubes, int numCubes, nbhd *nbhds, int *neighborArray, cube *neighborhood, int maxNbrs, int *trinaryList, nbhd* cubeNbhd, int* testIndex, int* cubePtsArray,int dim, double inner, double *minDistance, double *maxGradient);
    ...
    double minDistance;
    double maxGradient;
    currEnergy=sEnGrad(points, oldGrad, s, radius, cornera, cornerb, numpts, cubes, numCubes, nbhds, neighborArray, neighborhood, maxNbrs, trinaryList, &cubeNbhd,testIndex,cubePtsArray,dim,inner, &minDistance, &maxGradient);
    ...
    for (i = 0; i <= iterations; i++){
        ...
        //time consuming part
        t0 = ConjGrad(points, newpoints, numpts, currEnergy, theDir, newGrad, oldGrad, t0, radius, cornera, cornerb, cubes, numCubes, neighborArray, maxNbrs, s, neighborhood, nbhds, writeTo, trinaryList, &cubeNbhd, testIndex,cubePtsArray,dim,inner,update,sEnGrad, &minDistance, &maxGradient);
        ...
        currEnergy=sEnergy(newpoints, s, radius, cornera, cornerb, numpts, cubes, numCubes, nbhds, neighborArray, neighborhood, maxNbrs, trinaryList, &cubeNbhd, testIndex, cubePtsArray,dim, &minDistance, &maxGradient);
